...
#include"stdio.h"
#include"string.h"
#define Analog_Input GPIO_PIN_0


#define Digital_Output GPIO_PIN_5
...
sprintf(&buffer[0], "CMD+MQTTPUB=sound/sensor,%d\r\n", n);
...
int main(void)
{
  ...
  char message[100];
    while (1)
    {
      // Read the analog input from pin A0 (in this case, it's PA0)
      HAL_ADC_Start(&hadc1);
      if (HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY) == HAL_OK)
      {
        // Get the analog value and check if it's above the threshold
        ...
        {
          // If sound is detected (analog value above threshold), turn on the LED on pin PA5
          HAL_GPIO_WritePin(GPIOA, Digital_Output, GPIO_PIN_SET);
          sensor_value=1;
       //   sprintf(message,"sound is detected...\n");
        ...
        {
          // If no sound is detected, turn off the LED on pin PA5
          HAL_GPIO_WritePin(GPIOA, Digital_Output, GPIO_PIN_RESET);
          sensor_value=0;
          //sprintf(message,"sound is not detected...\n");
...
void MQTT_Init()
{
	...
	/****CMD+MQTTCONCFG---->LED ***/
	//memset(&buffer[0],0x00,strlen(buffer));
	sprintf (&buffer[0], "CMD+MQTTCONCFG=3,mqtt-sarangammanindra-d92xdx,,,,,,,,,\r\n");
	HAL_UART_Transmit(&huart1, (uint8_t*)buffer, strlen(buffer), 1000);
	HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 1000);
	//memset(&buffer[0],0x00,strlen(buffer));
	//HAL_Delay(500);
